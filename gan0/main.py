...
def get_disc_loss(gen, disc, criterion, real, num_images, z_dim, device):
    ...
    verdict_fake = disc(fakes)
    loss_disc_fake = torch.squeeze(criterion(verdict_fake,torch.zeros_like(verdict_fake, device=device)))
    
    verdict_real = disc(real[:num_images, ...])
    loss_disc_real = torch.squeeze(criterion(verdict_real, torch.ones_like(verdict_real, device=device)))
